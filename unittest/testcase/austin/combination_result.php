...
class PUTest extends PHPUnit_Framework_TestCase {
	...
	public function _testSplit(){
		$s = '124556';
		$arr = str_split( $s );
		var_dump( $arr );
	}
	public function _testCombinSet(){
		
		$target_set = array( 2,4,5 );
		...
		for( $combin_index = 0; $combin_index < $combin_size; ++$combin_index ){
			...
			$l_set_str = implode('_', $l_val_set);
			$r_set_str = implode('_', $r_val_set);
...
// 		var_dump( $combin_val_set_arr );
	}
	/**
	 * 
	 */
	public function testGameTestAbc(){

		$target_set = array( 1, 2 , 3);
		$tar_val = 20; 
		
		sort( $target_set );
		$target_set = array_reverse( $target_set );
		
		$res = $this->get_result_arr( $target_set );
		for( $i = 0; $i < count($res['value_set']); ++$i){
			
			if( $tar_val == $res['value_set'][$i] ){
				echo "{$res['desc_set'][$i]}</br>";
			}
		}
// 		var_dump( $res['value_set'] );
	}
	...
	public function get_result_arr( $target_set, $op_result_set = array() ){
		...
		for( $combin_index = 0; $combin_index < $combin_size; ++$combin_index ){
			...
			var_dump( "==============cindex/csize  {$combin_index}:{$combin_size}" );
