	...
	public function dataCGI(){
		...
		return array(
			array(
					...
					'post' => array(
							'paymethod' => '61',
							'udid' => '00000000-62b1-fd46-317b-74d84365ca4c',							
...
// 		$request_header_str_set[] = "Content-Type: application/json";
	...
	public function dataCGIUnsub(){
		$target_domain = TARGET_DOMAIN;
		return array(
				array(
						'cgi' => "http://{$target_domain}/payment/unsubscribe",
						'post' => array(
								'paymethod' => '37',
								'udid' => '00000000-62b1-fd46-317b-74d84365ca4c',
								'channel_id' => 'AL0S0N10000',
	...
	/**
	 * @dataProvider dataCGIUnsub
	 *
	 * @param unknown $str_target_cgi
	 * @param unknown $expect_result
	 */
	public function testCGIUnsubscribe( $str_target_cgi, $post_arr, $expect_result, $desc = '' ){
		$obj_curl = curl_init();
		curl_setopt( $obj_curl, CURLOPT_URL, $str_target_cgi );
		curl_setopt( $obj_curl, CURLOPT_RETURNTRANSFER, 1 );
